...
logical_idx_pval=~cellfun(@isempty,regexpi(anova_summary_table.Properties.VariableNames,'pval'));
pos_idx_pval=find(logical_idx_pval);
for n=1:numel(pos_idx_underscore)
    anova_summary_table.Properties.VariableNames{pos_idx_pval(n)}=strrep(anova_summary_table.Properties.VariableNames{pos_idx_pval(n)},'pval','P-Value');
end
