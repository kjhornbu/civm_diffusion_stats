...
%     end
    ...
    for n=1:numel(group_names)
        xCenter=mean(data_subset.X1(group_name_idx==n));
        yCenter=mean(data_subset.X2(group_name_idx==n));
        xRadius=std(data_subset.X1(group_name_idx==n));
        yRadius=std(data_subset.X2(group_name_idx==n));

        idx_color=mod(n,size(myColorOrder,1));
        if floor(n/size(myColorOrder,1)) > 0 && idx_color == 0
            plot_STD_ring(xCenter,yCenter,xRadius,yRadius,myColorOrder(size(myColorOrder,1),:));
        else
            plot_STD_ring(xCenter,yCenter,xRadius,yRadius,myColorOrder(idx_color,:));
        end
    end
