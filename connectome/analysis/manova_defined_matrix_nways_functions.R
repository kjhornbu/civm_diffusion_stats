...
format_manova <- function(df,
  ...
  col_dependant_variable <-
    which(grepl("^X", names(df))) # column indices for the embedding
  dependant_variable <- colnames(df)[col_dependant_variable]
  
  #Check for missing data
  
  df.statistical_output <- manova_significance_testing(
    df,
    number_of_vertices,
    number_of_sources_of_variation,
    col_dependant_variable,
...
manova_significance_testing <- function(df,
                                        number_of_vertices,
                                        number_of_sources_of_variation,
                                        col_dependant_variable,
                                        dependant_variable,
  ...
  form <- paste0("cbind(",
                 paste(dependant_variable, collapse = ", "),
                 ") ~ ",
                 independent_variable)
