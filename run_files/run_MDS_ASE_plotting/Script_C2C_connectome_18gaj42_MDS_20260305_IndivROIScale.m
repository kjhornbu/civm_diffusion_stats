...
working_folder='Z:\All_Staff\18.gaj.42\FullAnalysis_20260224\MDS_Plots';
...
%% Regional MDS --- strain separated

idx_group=cellfun(@isempty,regexpi(output_regional.hold,'All'));
temp_group = output_regional(idx_group ,:);

[strain,~,idx]=unique(Data_regional.group2);

for o=1:height(strain)
    idx_strain=~cellfun(@isempty,regexpi(temp_group.hold,strain{o}));
    temp_out_strain=temp_group(idx_strain,:);

    for n=1:numel(values_T)

        idx=temp_out_strain.vertex==values_T(n);
        idx_indiv=Data_regional.vertex==values_T(n);

        temp_out = temp_out_strain(idx ,:);
        ...
        for m=1:height(temp_out)
			...
			strain_idx=~cellfun(@isempty,regexpi(Data_regional.group2,strain{o}));

			temp_indiv_data = Data_regional(age_idx&strain_idx&idx_indiv,:); %& idx_indiv
        ...
        if values_T(n)>180
            print(f, fullfile(working_folder,'REGIONAL',strcat('Regional_MDS_plot_',strain{o},'_ROI_',num2str(values_T(n)-180+1000),'.svg')),'-dsvg','-vector');
        else
            print(f, fullfile(working_folder,'REGIONAL',strcat('Regional_MDS_plot_',strain{o},'_ROI_',num2str(values_T(n)),'.svg')),'-dsvg','-vector');
        end
    end
end

%% Regional MDS --- strain separated with C2C
...
for o=1:height(strain)
    ...
    for n=1:numel(values_T)
        ...
        if values_T(n)>180
            print(f, fullfile(working_folder,'REGIONAL',strcat('Regional_MDS_plot_',strain{o},'_ROI_',num2str(values_T(n)-180+1000),'_withCentroid2Centroid.svg')),'-dsvg','-vector');
        else
            print(f, fullfile(working_folder,'REGIONAL',strcat('Regional_MDS_plot_',strain{o},'_ROI_',num2str(values_T(n)),'_withCentroid2Centroid.svg')),'-dsvg','-vector');
        end
    end
end
...
for n=1:numel(values_T)
    ...
    for m=1:height(temp_out)
        ...
        if strcmp(temp_out.compare{m},'Old')
            plot(temp_indiv_data.X1/s_regional(1,values_T(n)),temp_indiv_data.X2/s_regional(2,values_T(n)),'o','MarkerFaceColor', [0.4660 0.6740 0.1880],'MarkerEdgeColor',[1 1 1],'MarkerSize',6);
        elseif strcmp(temp_out.compare{m},'Young')
            plot(temp_indiv_data.X1/s_regional(1,values_T(n)),temp_indiv_data.X2/s_regional(2,values_T(n)),'o','MarkerEdgeColor',[0.4940 0.1840 0.5560],'MarkerSize',6);
        end
...
for n=1:height(strain)
    ...
    print(f, fullfile(working_folder,'GLOBAL',strcat('Global_MDS_plot',strain{n},'.svg')),'-dsvg','-vector');
...
for n=1:height(strain)
    ...
    print(f, fullfile(working_folder,'GLOBAL',strcat('Global_MDS_plot',strain{n},'_withCentroid2Centroid.svg')),'-dsvg','-vector');

end

civm_write_table(output_regional,fullfile(working_folder,'REGIONAL_MDS_centroid_to_centroid_Distance_All_Strains+All.csv'));
civm_write_table(output_global_overall,fullfile(working_folder,'GLOBAL_MDS_centroid_to_centroid_Distance_All_Strains+All.csv'));