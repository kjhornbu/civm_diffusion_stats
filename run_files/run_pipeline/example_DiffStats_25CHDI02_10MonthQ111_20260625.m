...
project_code={'25.chdi.02'};
studySuffix={};
stratID={'10MonthQ111'};
dates.download={'2026-06-17'};
dates.clean=dates.download;
dates.process=dates.clean;
names.google_cap=sprintf('%s_%s_fixed_%s.txt', ... 
    uncell(project_code), uncell(stratID), uncell(dates.download));
...
dirs.statistical_view_test=uncell(fullfile(getenv('BIGGUS_DISKUS'),'test',names.study_dir,'statistical_view_REVERSION4'));
...
prepare_diffstat_results(dirs.statistical_view_test, files.stat_config, ...
    'stratification_output_name',uncell(names.stats_run), ...
    'slice_selection', [1, 2, 3, 4],...
