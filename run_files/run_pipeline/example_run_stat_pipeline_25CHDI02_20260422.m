close all;
clear variables;

%% Preliminaries
studyID='24.chdi.01';
project_dir='Z:\All_Staff\25.chdi.02\';
%google_doc=fullfile(project_dir,"google_sheets","25.chdi.02_scansheet_20260422.tsv");
google_doc=fullfile('B:\24.chdi.01-PHASE2','KH_Google_Sheet_Caps','24.chdi.01 - MRI record for Phase II-2026-01-10.tsv');
cleaned_google_doc_path=fullfile(project_dir,"google_sheets","Edited_GoogleSheetZQ175_2026_04_22.txt");
statSaveDir=fullfile(project_dir,'12Month_ZQ175_20260422');

dataframe_path=fullfile(project_dir,"25.chdi.02_ZQ175_DataFrame_20260422.txt");
config_file='';
polished_sheets=fullfile(project_dir,'polished_sheets'); %where the polished sheets will be saved 
project_research_archive{1}=fullfile('A:/',studyID,'research');
atlas_ontology_path=fullfile(getenv("WORKSTATION_HOME"),'static_data','atlas','symmetric15um','labels','RCCF','symmetric15um_RCCF_labels_lookup.txt'); 
%  'googleDocPath',google_doc,...
% 'cleanedGoogleDocPath',cleaned_google_doc_path,...
studyParams={
  studyID,...
  statSaveDir,...
  'configFile',config_file,...
  'googleDocPath',google_doc,...
  'cleanedGoogleDocPath',cleaned_google_doc_path...
  'dataframePath',dataframe_path...
  'overrideLabelLUT',atlas_ontology_path,...
  'researchArchivePath',project_research_archive,...
  'polishedSheetPath',polished_sheets,...
  'assumeNLSAM',true ...
  'allowMissing',true};
%'analysisPipelineType', list2cell('Scalar')};
civm_diffusion_stats(studyParams{:});